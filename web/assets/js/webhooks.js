...
const celEditor = new AceEditor("webhooks-input");
...
function run() {
  ...
  const result = webhookEval(expression, dataOriginal, dataUpdated, dataRequest, dataAuthorizer);
  ...
  WebAssembly.instantiate(buffer, go.importObject)
    .then((result) => {
      ...
      document.getElementById("output").placeholder =
        "Press 'Run' to evaluate the Webhooks.";
    })
    .catch((err) => {
      console.error(err);
    });
...
fetch("../assets/webhooks_data.json")
  ...
  .then(({ examples, versions }) => {
    ...
    examples.forEach((example) => {
      ...
      if (example.name === "default") {
        if (!urlParams.has("content")) {
          celEditor.setValue(example.webhooks, -1);
